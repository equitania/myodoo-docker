#!/bin/bash
# Erzeugt man eine NextCloud/OwnCloud nginx Konfiguration inkl. SSL
# Skript muss mit root-Rechten ausgefuehrt werden
##############################################################################
#
...
if [ "$myport" = "" ]; then
  echo "Insert the odoo port | Geben Sie den Port fuer NextCloud ein:"
  read myport
fi
