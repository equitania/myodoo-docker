#!/bin/bash
# Install all Python 3.6 Libs for Odoo 12
# Version 2.0.3 - Stand 29.04.2019
##############################################################################
#
...
virtualenv -p /usr/bin/python3.6 $mypython
...
python3.6 -m pip install --no-cache-dir wheel --upgrade \
    ...
    && python3.6 -m pip install --no-cache-dir chardet==3.0.4 \
    ...
    && python3.6 -m pip install --no-cache-dir gevent==1.3.4 \
    && python3.6 -m pip install --no-cache-dir greenlet==0.4.13 \
    && python3.6 -m pip install --no-cache-dir html2text==2016.9.19 \
    && python3.6 -m pip install --no-cache-dir Jinja2==2.10.1 \
    && python3.6 -m pip install --no-cache-dir libsass==0.12.3 \
    && python3.6 -m pip install --no-cache-dir lxml==4.2.3 \
    ...
    && python3.6 -m pip install --no-cache-dir PyYAML==3.13 \
    ...
    && python3.6 -m pip install --no-cache-dir Werkzeug==0.11.15 \
    ...
    && python3.6 -m pip install --no-cache-dir python-slugify \
    && python3.6 -m pip install --no-cache-dir parse-accept-language \
