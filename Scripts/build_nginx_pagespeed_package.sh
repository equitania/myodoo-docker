#!/bin/bash
# Install nginx with pagedspeed
# Script must run with mit root-rights
# Version 1.0.0 - Stand 07.05.2018
##############################################################################
#
...
apt-get update && apt-get -y dist-upgrade && apt-get install -y sudo build-essential zlib1g-dev unzip uuid-dev curl libpcre3-dev wget libssl-dev tzdata && \
...
cp $PWD/nginx.service /lib/systemd/system/
...
cp $PWD/nginx-conf/nginx.conf /etc/nginx/
