...
- **server-readiness.py** (v1.9.1) — beantwortet „ist dieser Server auf Stand, und was fehlt noch?“ mit einer Ampel-Liste aus 17 Prüfungen (Wartungs-Cron, logrotate, doppelte Cron-Einträge, Log-Größen, Backup-Alter/-Konfiguration/-Plattenplatz, **Update-Konfiguration**, Docker-Storage-Driver, **Virenscanner-Ausnahme für `/var/lib/docker/`**, nginx-systemd-Drop-in, nginx-listen-Ziele, certbot-Fenster, Skript-Stände, Build-Cache). Fehlt `docker2update.yaml`, sagt der Bericht das seit v1.6.0 genauso deutlich wie beim Backup — vorher war der Verlust dieser Datei unsichtbar, und ein Server ohne sie kann kein `doup` fahren. Zu jedem Befund gehört genau ein kopierfertiger Fix-Befehl. **Rein lesend** — verändert nichts. Läuft automatisch am Ende jedes `ups`-Laufs (`--brief`), auf Zuruf per `chk`, und wöchentlich montags 06:00 per Cron (`--quiet`, meldet sich nur bei Abweichung)
...
- [docs/MANUAL_DOCKER_UPDATE_GUIDE.md](docs/MANUAL_DOCKER_UPDATE_GUIDE.md) — manuelles Container-Update (veraltet, nur historisch)
...
- **server-readiness.py** (v1.9.1) — answers "is this server up to date, and what is still missing?" with a traffic-light list of 17 checks (maintenance cron, logrotate, duplicate cron entries, log sizes, backup age/config/disk space, **update config**, Docker storage driver, **virus-scanner exclusion for `/var/lib/docker/`**, nginx systemd drop-in, nginx listen targets, certbot window, script staleness, build cache). A missing `docker2update.yaml` is reported as plainly as a missing backup config since v1.6.0 — before that the loss of this file was invisible, and a server without it cannot run `doup` at all. Every finding carries exactly one copy-paste fix command. **Read-only** — changes nothing. Runs automatically at the end of every `ups` run (`--brief`), on demand via `chk`, and weekly on Mondays at 06:00 via cron (`--quiet`, speaks up only on drift)
...
- [docs/MANUAL_DOCKER_UPDATE_GUIDE.md](docs/MANUAL_DOCKER_UPDATE_GUIDE.md) — manual container update (outdated, historical only)
