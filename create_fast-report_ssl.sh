#!/bin/bash
# Erzeugt man eine nginx Konfiguration für Fast-Report .net Core unter Docker inkl. SSL
...
if [ "$myport" = "" ]; then
  echo "Insert the odoo port | Geben Sie den Port für Fast-Report ein:"
  read myport
fi
...
if [ "$myip" != "" ] || [ "$mydomain" != "" ] || [ "$myport" != "" ] || [ "$mykey" != "" ]  || [ "$mycrt" != "" ]; then
  cp  $myscriptpath"/fast_report_template.conf" $myserverpath"/$mydomain.conf"
